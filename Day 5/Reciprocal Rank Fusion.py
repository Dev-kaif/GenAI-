...
def rrf_fusion(all_retrievals, k=60):
    """
    all_retrievals: List of Lists of Documents retrieved for each expanded query
    Returns: List of Documents sorted by RRF scores
    """
    scores = defaultdict(float)
    doc_map = {}

    for retrieval in all_retrievals:
        for rank, doc in enumerate(retrieval):
            key = doc.page_content.strip()
            scores[key] += 1 / (k + rank + 1)
            doc_map[key] = doc  # Keep the latest doc object for metadata

    # Sort by descending RRF score
    sorted_docs = sorted(scores.items(), key=lambda x: x[1], reverse=True)
    final_docs = [doc_map[key] for key, _ in sorted_docs]
    return final_docs
...
def __main__():
    while True:
        ...
        all_retrievals = []
        for expanded_query in multiple_result:
            retrieved = do_similarity_serach_for_multiple(expanded_query)
            all_retrievals.append(retrieved)

        fused_docs = rrf_fusion(all_retrievals, k=60)
